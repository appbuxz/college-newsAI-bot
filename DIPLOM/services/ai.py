...
async def answer_student_question(question: str, student_name: str, group_name: str, announcements: list) -> str:
    ...
    try:
        system_prompt = f"""Ты помощник студента учебного заведения.
Студент: {student_name}, группа: {group_name}.

Вот последние объявления для его группы:
{context if context else "Объявлений пока нет."}

Отвечай на вопрос студента коротко и по делу, опираясь на объявления выше.
Если информации нет — честно скажи что не знаешь и посоветуй обратиться к преподавателю.
Не придумывай информацию. Отвечай только на русском языке."""
